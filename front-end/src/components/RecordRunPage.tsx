...
import { Plus, MapPin, Clock, Zap, Flame, Calendar, Timer, ArrowLeft } from 'lucide-react';
...
function RecordRunPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Combine date and startTime for startTime
    const startDateTime = formData.startTime 
      ? `${formData.date}T${formData.startTime}:00` 
      : `${formData.date}T08:00:00`;
    
    // Calculate finishTime based on duration
    const startDate = new Date(startDateTime);
    const finishDate = new Date(startDate.getTime() + totalTime * 1000);
    const finishDateTime = formData.finishTime 
      ? `${formData.date}T${formData.finishTime}:00` 
    ...
    try {
      ...
      if (!res.success) {
        setMessage(res.message || 'Failed to record run');
      } else {
        setMessage('Run recorded successfully!');
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          <div className="bg-gradient-to-r from-blue-300 via-blue-500 to-blue-900 p-8">
            <div className="flex items-center text-white">
              ...
              <div>
                <h1 className="text-3xl font-bold">Record New Run</h1>
                <p className="text-blue-100 mt-1">Track your fitness journey</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Finish Time (optional)</label>
                  <div className="relative">
                    <Clock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
                    <input
                      type="time"
