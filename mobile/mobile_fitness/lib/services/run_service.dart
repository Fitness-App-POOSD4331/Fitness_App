...
class RunService {
  static const String _baseUrl = 'http://smallproject.shop:5001/api/runs';
  ...
  // 4. PUT: Update Run Description (FIXED: Changed from PATCH to PUT)
  Future<Run> updateRunDescription(String runId, String newDescription) async {
    final headers = await _getAuthHeaders();
    ...
    if (response.statusCode == 200) {
      ...
      final errorBody = jsonDecode(response.body);
      throw Exception(errorBody['message'] ?? 'Failed to update run description');
    }
  }
}